...
object ConfigError {
  ...
  implicit val showErrorType: Show[ErrorType] = Show.shows[ErrorType]({
    ...
    case ParseError(provided, t) =>
      s"The provided value $provided cannot be converted to the type $t."
    case InvalidValue(provided, expected) =>
      s"Invalid value. Provided: $provided, Expected: $expected"
  })

  implicit val showConfigError: Show[ConfigError] =
    Show.shows[ConfigError](a => s"Error reading configuration. ${a.key}. Cause: ${a.error.shows}")
}
